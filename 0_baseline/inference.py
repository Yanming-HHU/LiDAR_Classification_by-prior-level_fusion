...
# sys.path.append(BASE_DIR) # model
# sys.path.append(os.path.join(ROOT_DIR, '')) # no, really model
...
def parse_args(argv):
    ...
    parser.add_argument('--output_type', type=OutputType, choices=list(OutputType), default=OutputType.BOTH)
...
def inference():
    ...
    files = [os.path.join(FLAGS.input_path, 'Vaihingen3D_EVAL_WITH_REF_new.txt')]
...
def prep_pset(pset):
    data64 = np.stack([pset.x,pset.y,pset.z],axis=1)
...
def get_batch(dataset, start_idx, end_idx):
    ...
    batch_raw = np.zeros((rsize, NUM_POINT, NUM_DIMENSIONS), dtype=np.float64)
    ...
    with tf.Graph().as_default():
        ...
        while True:
            ...
            for k in range(len(batch_list)):
                ...
                # predict the probability of the class using softmax
                ...
                if batch_raw.shape[0] != BATCH_SIZE:
                    pred_prob = pred_prob[0:batch_raw.shape[0],:]
...
def post_processor(output_queue):
    while True:
        ...
        all_probs = out_data[2]

        logging.info('Post-processing {}'.format(pset.filename))
        # calculate the probability of pset using k-nn classifier 
        point_cloud = np.stack((pset.x, pset.y, pset.z), axis=1)
        kdt = KDTree(all_points, metric='euclidean')
        ind = kdt.query(point_cloud, k=FLAGS.n_angles*4+1,return_distance=False) # Note: number of votes is FLAGS.n_angles*4+1, where 4 comes from splitting the point cloud (nominal number of overlapping subtiles per point before rotations)
        probs_neighbor = all_probs[ind]
        point_cloud_prob = np.average(probs_neighbor, axis=1)
        np.save(os.path.join(FLAGS.output_path, pset.filename+'_.npy'), point_cloud)
        np.save(os.path.join(FLAGS.output_path, pset.filename+'_probs.npy'), point_cloud_prob)
