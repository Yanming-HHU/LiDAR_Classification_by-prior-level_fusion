...
def parse_args(argv):
    ...
    parser.add_argument('--output_path', default='/result_output/4_decision_level/data/ds2010', help='Output path')
...
if __name__ == '__main__':
    ...
    for split in ['train', 'test']:
        data = ISPRSDataset('../1_xyz/isprs_data', split='test')
        ...
        if split == 'test':
            ...
            whole_data = np.loadtxt('../data/Vaihingen3D_training.txt')
        pred_probs = predict_for_whole(whole_data, all_points, all_probs, epoch)
        np.save(os.path.join(flags.output_path, 'prob_point_{}.npy'.format(split)), pred_probs)
    logging.info('Done')
