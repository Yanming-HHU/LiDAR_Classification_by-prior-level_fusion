...
parser.add_argument('--output_dir', default='/result_output/5_prior_level/data', help='path to the output')
...
def raster_prob(raster_file, model_path=None, batch_size=BATCH_SIZE, 
    ...
    raster_prob = raster_prob.transpose(2,0,1) / np.sum(raster_prob, axis=-1)
    return raster_prob.transpose(1,2,0)
...
if __name__ == "__main__":
    ...
    for split in ['train', 'test']:
        ...
        np.save(os.path.join(OUTPUT_DIR, 'prob_raster_{}'.format(split)), prob)
